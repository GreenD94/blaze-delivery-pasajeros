...
import 'package:taxibooking/language/AppLocalizations.dart';
...
abstract class _AppStore with Store {
  ...
  @action
  Future<void> setEnabledReferrals(bool val) async {
    referralsEnabled = val;
  }

  @action
  Future<void> setExchangeRate(num amount) async {
    exchangeRate = amount;
  }
  
  @action
  Future<void> setAuctionEnabled(bool enabled) async {
    log('[modality] auction: $enabled');
    auctionEnabled = enabled;
  }

  @action
  Future<void> setExpressEnabled(bool enabled) async {
    log('[modality] express: $enabled');
    expressEnabled = enabled;
  }

  @action
  void setRideModality(String modality) {
    log('[modality] selected: $modality');
    selectedRideModality = modality;
  }

  @action
  Future<void> setCancellationReasons(List<dynamic> reasons) async {
