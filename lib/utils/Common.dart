...
InputDecoration inputDecoration(BuildContext context, {String? label, Widget? prefixIcon, Widget? suffixIcon}) {
  return InputDecoration(
    ...
    border: OutlineInputBorder(
        borderRadius: BorderRadius.circular(defaultRadius), borderSide: BorderSide(color: primaryColor.withOpacity(0.1))),
...
Widget totalCount({String? title, String? subTitle, String? description}) {
  return Row(
    ...
    children: [
      ...
      Text(appStore.currencyPosition == LEFT ? '$subTitle' : '$subTitle',
...
Future<void> playNotificationSound() async {
  ...
  int soundId = await rootBundle.load("sounds/notification.wav").then((ByteData soundData) {
    return pool.load(soundData);
  });
  int streamId = await pool.play(soundId);
}

String toCurrency(num amount, {bool includeRate = true}) {

  num exchangeRate = appStore.exchangeRate ?? 0.0;
  num rate = amount * exchangeRate;
  amount = double.parse(amount.toStringAsFixed(fixedDecimal));
  rate = double.parse(rate.toStringAsFixed(fixedDecimal));
  String rateLabel = includeRate ? '(Bs. $rate)' : '';
  return appStore.currencyPosition == LEFT ? '${appStore.currencyCode} $amount $rateLabel' : '$amount ${appStore.currencyCode} $rateLabel';
}
