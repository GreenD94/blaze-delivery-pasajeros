...
class OrderDetailScreenState extends State<OrderDetailScreen> {
  ...
  void init() async {
    ...
    await getWalletList(page: 1).then((value) {
      walletBalance = value.walletBalance!.totalAmount ?? 0;
      setState(() {});
    }).catchError((error) {
      log(error.toString());
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        children: [
          currentData != null && riderModel != null
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            ...
                            if (currentData!.payment!.paymentType == 'cash')
                              Column(
                                children: [
                                  SizedBox(height: 16),
                                  totalCount(title: 'Wallet (-)', description: '', subTitle: toCurrency(walletBalance)),
                                ],
                              ),
                            SizedBox(height: 16),
                            // totalCount(
                            ...
                            //     description: '',
                            //     subTitle: riderModel!.perDistanceCharge.toString()),
                            // SizedBox(height: 16),
                            // totalCount(
                            //     title: language.duration, description: '', subTitle: '${riderModel!.perMinuteDriveCharge}'),
                            // SizedBox(height: 16),
                            // totalCount(
                            //     title: language.waitTime, description: '', subTitle: '${riderModel!.perMinuteWaitingCharge}'),
                            // SizedBox(height: 16),
                            if (paymentData != null)
                              totalCount(title: language.tip, description: '', subTitle: toCurrency(paymentData!.driverTips!) ),
                            ...
                            if (riderModel!.couponData != null && riderModel!.couponDiscount != 0)
                              Row(
                                ...
                                children: [
                                  ...
                                  Text(
                                      toCurrency(riderModel!.couponDiscount ?? 0),
                            ...
                            if (currentData!.payment!.paymentType == 'cash')
                              totalCount(
                                  ...
                                  description: '',
                                  subTitle: toCurrency(
                                      ((riderModel!.subtotal ?? 0) - (walletBalance)) < 0 ? 0 : (riderModel!.subtotal ?? 0) - (walletBalance))),
                            if (currentData!.payment!.paymentType != 'cash')
                              totalCount(title: language.total, description: '', subTitle: toCurrency(riderModel!.subtotal!))
                          ],
                        ),
                      ),
                      SizedBox(height: 16),
                                ...
                                Expanded(
                                  child: inkWellWidget(
                                    ...
                                    child: scheduleOptionWidget(
                                        context, !isCashPayment, 'images/ic_credit_card.png', language.wallet),
                                  ),
                                ),
                              ],
                            ),
          ...
          //     child: Center(child: Lottie.asset(paymentSuccessful, width: 400, height: 400, fit: BoxFit.contain))),
        ],
      ),
    );
  }
  ...
  String? getButtonText() {
    if (currentData!.payment!.paymentStatus == COMPLETED) {
      return language.continueNewRide;
        ...
        (currentData!.payment!.paymentType == 'cash' || currentData!.payment!.paymentType == 'mobile-payment')) {
