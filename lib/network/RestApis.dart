...
Future<LDBaseResponse> cancelDriverProposal({required Map request, int? rideId}) async {
  return LDBaseResponse.fromJson(await handleResponse(await buildHttpResponse('cancel-proposal/$rideId', method: HttpMethod.POST, request: request)));
}
...
Future<GoogleMapSearchModel> searchAddressRequest({String? search, LatLng? coordinates, int? radius}) async {
  final baseUrl = 'https://maps.googleapis.com/maps/api/place/autocomplete/json';
  final String lat = coordinates?.latitude.toString() ?? '0.00';
  final String lng = coordinates?.longitude.toString() ?? '0.00';

  final url = Uri.parse('$baseUrl?input=$search&types=geocode&location=$lat,$lng&radius=$radius&components=country:ve|country:ec&key=$googleMapAPIKey');
  // final response = await http.get(url);

  return GoogleMapSearchModel.fromJson(await handleResponse(await buildHttpResponse(url.toString(),
      // 'https://maps.googleapis.com/maps/api/place/autocomplete/json?input=$search&key=$googleMapAPIKey',
      method: HttpMethod.GET)));
}

Future<GooglePlaceIdModel> searchAddressRequestPlaceId({String? placeId}) async {
  return GooglePlaceIdModel.fromJson(await handleResponse(await buildHttpResponse('https://maps.googleapis.com/maps/api/place/details/json?place_id=$placeId&key=$googleMapAPIKey', method: HttpMethod.GET)));
}
...
Future<LDBaseResponse> logoutApi() async {
  return LDBaseResponse.fromJson(await handleResponse(await buildHttpResponse('logout?clear=player_id', method: HttpMethod.POST)));
}

Future<ReferCodeResponse> checkRefCode({required String refCode}) async {
  Map req = {
    'ref_code': refCode,
  };

  return ReferCodeResponse.fromJson(await handleResponse(await buildHttpResponse('check-ref-code', method: HttpMethod.POST, request: req)));
  // return buildHttpResponse('check-ref-code', method: HttpMethod.POST, request: req);
}

Future getRefCode() async {
  return handleResponse(await buildHttpResponse('referral-info', method: HttpMethod.GET));
}

Future checkReferralsEnabled() async {
  return handleResponse(await buildHttpResponse('referrals-enabled', method: HttpMethod.GET));
}

Future getBanners() async {
  return handleResponse(await buildHttpResponse('banners', method: HttpMethod.GET));
}
